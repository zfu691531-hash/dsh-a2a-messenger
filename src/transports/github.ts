...
export class GitHubTransport implements Transport {
  ...
  constructor(private readonly opts: GitHubTransportOptions) {
  ...
  async peers(): Promise<TransportPeer[]> {
    ...
    return raw
      .map((item) => (item as Record<string, unknown>).login)
  ...
  private async run(): Promise<void> {
    ...
    try {
      ...
      if (!this.stopped) this.timer = setTimeout(() => void this.run(), this.pollIntervalMs)
      return
    }
    await this.pollLoop()
  }
  ...
  private normalizeComment(raw: unknown, channel: string): IncomingMessage | undefined {
    ...
    const ts =
      typeof r.created_at === 'string' ? Date.parse(r.created_at) || Date.now() : Date.now()
  ...
  private async request(method: string, path: string, body?: unknown): Promise<unknown> {
    ...
    try {
      ...
      throw new GitHubTransportError(`github unreachable: ${(err as Error).message}`)
    }
    if (!res.ok) {
      const text = await res.text().catch(() => '')
      throw new GitHubTransportError(
        `HTTP ${res.status} on ${method} ${path}: ${text.slice(0, 200)}`,
        res.status,
      )
    }
  ...
  private loadCursor(): string {
    ...
    if (file && existsSync(file)) {
      try {
        const raw = JSON.parse(readFileSync(file, 'utf8')) as { cursor?: string }
        if (typeof raw.cursor === 'string' && raw.cursor.length > 0) return raw.cursor
      } catch {
