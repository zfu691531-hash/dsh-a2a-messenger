import { existsSync, mkdirSync, readFileSync, renameSync, writeFileSync } from 'node:fs'
import { dirname } from 'node:path'
import type { IncomingMessage, QuarantinedMessage, QuarantineStatus } from './types.js'

export type AddResult = 'added' | 'duplicate' | 'rejected-too-large' | 'rejected-inbox-full'

export interface QuarantineInboxOptions {
  /** Maximum number of pending messages held at once. */
  maxPending?: number
  /** Maximum accepted content size per message, in bytes. */
  maxContentBytes?: number
  /** Maximum number of decided (accepted/rejected) records kept for dedup. */
  maxDecided?: number
}

/**
 * Persistent quarantine inbox. Incoming messages are stored here and are
 * NEVER model-visible until the local user explicitly accepts them.
 * Backed by a single JSON file written atomically (tmp file + rename).
 */
export class QuarantineInbox {
  private readonly items = new Map<string, QuarantinedMessage>()
  private readonly maxPending: number
  private readonly maxContentBytes: number
  private readonly maxDecided: number

  private constructor(
    private readonly filePath: string,
    options: QuarantineInboxOptions,
  ) {
    this.maxPending = options.maxPending ?? 200
    this.maxContentBytes = options.maxContentBytes ?? 64 * 1024
    this.maxDecided = options.maxDecided ?? 500
  }

  static open(filePath: string, options: QuarantineInboxOptions = {}): QuarantineInbox {
    const inbox = new QuarantineInbox(filePath, options)
    if (existsSync(filePath)) {
      try {
        const raw = JSON.parse(readFileSync(filePath, 'utf8')) as { messages?: unknown[] }
        if (Array.isArray(raw.messages)) {
          for (const item of raw.messages) {
            const m = item as QuarantinedMessage
            if (m && typeof m.id === 'string' && typeof m.content === 'string') {
              inbox.items.set(m.id, m)
            }
          }
        }
      } catch {
        // Corrupted file: start fresh; the next persist overwrites it.
      }
    }
    return inbox
  }

  add(msg: IncomingMessage): AddResult {
    if (this.items.has(msg.id)) return 'duplicate'
    if (Buffer.byteLength(msg.content, 'utf8') > this.maxContentBytes) return 'rejected-too-large'
    if (this.pendingCount() >= this.maxPending) return 'rejected-inbox-full'
    this.items.set(msg.id, { ...msg, receivedAt: Date.now(), status: 'pending' })
    this.persist()
    return 'added'
  }

  get(id: string): QuarantinedMessage | undefined {
    return this.items.get(id)
  }

  listPending(): QuarantinedMessage[] {
    return [...this.items.values()]
      .filter((m) => m.status === 'pending')
      .sort((a, b) => a.receivedAt - b.receivedAt)
  }

  pendingCount(): number {
    let n = 0
    for (const m of this.items.values()) if (m.status === 'pending') n++
    return n
  }

  accept(id: string): QuarantinedMessage | undefined {
    return this.decide(id, 'accepted')
  }

  reject(id: string): QuarantinedMessage | undefined {
    return this.decide(id, 'rejected')
  }

  /** Decide every pending message at once; returns the affected messages. */
  decideAll(status: Exclude<QuarantineStatus, 'pending'>): QuarantinedMessage[] {
    const affected: QuarantinedMessage[] = []
    for (const m of this.listPending()) {
      m.status = status
      m.decidedAt = Date.now()
      affected.push(m)
    }
    if (affected.length > 0) this.persist()
    return affected
  }

  private decide(id: string, status: Exclude<QuarantineStatus, 'pending'>): QuarantinedMessage | undefined {
    const m = this.items.get(id)
    if (!m || m.status !== 'pending') return undefined
    m.status = status
    m.decidedAt = Date.now()
    this.persist()
    return m
  }

  private persist(): void {
    this.pruneDecided()
    mkdirSync(dirname(this.filePath), { recursive: true })
    const payload = JSON.stringify({ version: 1, messages: [...this.items.values()] }, null, 2)
    const tmp = `${this.filePath}.tmp`
    writeFileSync(tmp, payload, 'utf8')
    renameSync(tmp, this.filePath)
  }

  private pruneDecided(): void {
    const decided = [...this.items.values()]
      .filter((m) => m.status !== 'pending')
      .sort((a, b) => (a.decidedAt ?? 0) - (b.decidedAt ?? 0))
    const excess = decided.length - this.maxDecided
    for (let i = 0; i < excess; i++) {
      const victim = decided[i]
      if (victim) this.items.delete(victim.id)
    }
  }
}
