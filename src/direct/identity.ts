...
export function openDirectIdentity(filePath: string): DirectIdentity {
  if (existsSync(filePath)) return parseStoredIdentity(readFileSync(filePath, 'utf8'))
...
function parseStoredIdentity(text: string): DirectIdentity {
  let raw: unknown
  try {
    raw = JSON.parse(text)
  } catch {
  ...
  const stored = raw as Partial<StoredIdentity>
  if (
    stored.version !== 1 ||
