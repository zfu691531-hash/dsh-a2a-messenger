    ...
    async send(input) {
        if (input.to) {
            if (!input.content.trim().startsWith('DSH1-')) {
                throw new GitHubTransportError('github readable direct messages are not supported; use a direct session or sealed mailbox encryption');
            }
        }
...
function matchingIssueNumbers(raw, title) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .flatMap((item) => {
