...
export function openDirectIdentity(filePath) {
    if (existsSync(filePath)) {
        ...
        if (parsed.migrated)
            persistIdentity(filePath, parsed.identity);
...
function persistIdentity(filePath, identity) {
...
function parseStoredIdentity(text) {
    ...
    try {
        ...
        const derivedEncryptionPublic = createPublicKey(encryptionPrivate)
            .export({ format: 'der', type: 'spki' })
            .toString('base64url');
        if (derivedEncryptionPublic !== encryptionPublicKey)
            throw new Error('encryption public/private key mismatch');
        return { identity: {
