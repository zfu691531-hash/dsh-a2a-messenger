import { createCipheriv, createDecipheriv, createPrivateKey, createPublicKey, diffieHellman, hkdfSync, randomBytes, randomUUID, } from 'node:crypto';
...
export class MailboxEnvelopeCodec {
    ...
    seal(input) {
        ...
        if (recipients.length === 0)
            throw new Error('sealed mailbox has no eligible recipients with encryption keys');
        const id = randomUUID();
        ...
        const unsigned = { v: 1, id, createdAt, expiresAt, sender, boxes };
        const envelope = {
            ...unsigned,
    ...
    open(value) {
        ...
        if (!envelope)
            throw new Error('invalid sealed mailbox envelope');
        if (envelope.sender.fingerprint === this.opts.identity.fingerprint)
            return undefined;
        const now = this.now();
        if (envelope.expiresAt < now)
            throw new Error('sealed mailbox envelope expired');
        if (envelope.createdAt > now + 5 * 60_000)
            throw new Error('sealed mailbox envelope is from the future');
        const contact = this.opts.contacts.find(envelope.sender.fingerprint);
        if (!contact || contact.trust === 'revoked')
            throw new Error(`sealed message from untrusted peer ${envelope.sender.fingerprint}`);
        if (contact.name !== envelope.sender.name ||
        ...
        if (this.seen.has(envelope.id))
            throw new Error('sealed mailbox envelope replayed');
        const payload = this.decrypt(box, envelope);
        if (payload.kind === 'rendezvous' &&
    ...
    recipientName(selector) {
        ...
        if (!contact || contact.trust === 'revoked')
            throw new Error(`active contact "${selector}" not found`);
        return contact.name;
    }
    resolveRecipients(input) {
        if (input.to) {
            ...
            if (!contact || contact.trust === 'revoked')
                throw new Error(`active contact "${input.to}" not found`);
            if (!contact.encryptionPublicKey)
                throw new Error(`contact "${contact.name}" has no encryption key; exchange a pairing card`);
            return [contact];
        }
        return this.opts.contacts.active().filter((contact) => Boolean(contact.encryptionPublicKey));
    }
    encryptFor(contact, id, createdAt, expiresAt, plaintext) {
    ...
    decrypt(box, envelope) {
        try {
            const key = this.sharedKey(envelope.sender.encryptionPublicKey, envelope.id, this.opts.identity.fingerprint);
            const decipher = createDecipheriv('aes-256-gcm', key, Buffer.from(box.iv, 'base64url'));
            decipher.setAAD(aad(envelope.id, envelope.createdAt, envelope.expiresAt, envelope.sender.fingerprint, this.opts.identity.fingerprint));
            decipher.setAuthTag(Buffer.from(box.tag, 'base64url'));
            const plaintext = Buffer.concat([
                decipher.update(Buffer.from(box.ciphertext, 'base64url')),
                decipher.final(),
            ]);
            const raw = JSON.parse(plaintext.toString('utf8'));
            ...
            throw new Error(`sealed mailbox decryption failed: ${err.message}`);
        }
    }
    sharedKey(remotePublicKey, id, recipient) {
        ...
        try {
            ...
            if (raw.seen && typeof raw.seen === 'object') {
                for (const [id, expiresAt] of Object.entries(raw.seen)) {
                    if (typeof expiresAt === 'number' && expiresAt >= this.now())
...
function aad(id, createdAt, expiresAt, sender, recipient) {
...
function decodeEnvelope(value) {
    ...
    if (!trimmed.startsWith(PREFIX) || trimmed.length > MAX_ENVELOPE_CHARS)
        return undefined;
    try {
        const raw = JSON.parse(Buffer.from(trimmed.slice(PREFIX.length), 'base64url').toString('utf8'));
        if (raw.v !== 1 ||
        ...
        for (const box of raw.boxes) {
            if (typeof box.recipient !== 'string' ||
