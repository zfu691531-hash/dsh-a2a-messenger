...
export function buildCommandDefs(deps) {
    ...
    const pair = {
        ...
        handler: () => {
            if (!deps.identity)
                return { kind: 'error', text: 'Pairing identity is unavailable.' };
            return {
                kind: 'success',
                text: [
    ...
    const pairAccept = {
        ...
        handler: (invocation) => {
            if (!deps.contacts)
                return { kind: 'error', text: 'Contact storage is unavailable.' };
            try {
                ...
                return { kind: 'error', text: `Pairing failed: ${err.message}` };
            }
        },
    };
    const contacts = {
        ...
        handler: () => {
            ...
            return {
                ...
                text: list.length === 0
                    ...
                    : list.map((contact) => `- ${contact.name}@${contact.deviceName} [${contact.trust}] ${contact.fingerprint}`).join('\n'),
            };
        },
    };
    const verifyContact = {
        ...
        handler: (invocation) => {
            ...
            if (!selector || !suffix)
                return { kind: 'error', text: 'Usage: /a2a-verify <name@device> <fingerprint-suffix>' };
            try {
                const contact = deps.contacts.verify(selector, suffix);
                return { kind: 'success', text: `Verified ${contact.name}@${contact.deviceName}.` };
            }
            catch (err) {
                return { kind: 'error', text: `Verify failed: ${err.message}` };
            }
        },
    };
    const untrust = {
        ...
        handler: (invocation) => {
            ...
            try {
                ...
                return { kind: 'error', text: `Revoke failed: ${err.message}` };
            }
        },
    };
    const call = {
        ...
        handler: async (invocation) => {
            if (!deps.rendezvous)
                return { kind: 'error', text: 'Automatic rendezvous is unavailable.' };
            const [peer, route] = invocation.rawInput.trim().split(/\s+/, 2);
            if (!peer)
                return { kind: 'error', text: 'Usage: /a2a-call <name@device> [route]' };
            try {
                ...
                return { kind: 'error', text: `Call failed: ${err.message}` };
            }
        },
    };
    const doctor = {
        ...
        handler: async () => {
            try {
                ...
                return { kind: 'error', text: `Diagnostics failed: ${err.message}` };
            }
        },
    };
    const receipts = {
